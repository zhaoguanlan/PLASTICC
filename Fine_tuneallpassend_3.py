...
class fine_tuneBERT(BERT):
    def __init__(self):
        ...
        self.fc1.weight = nn.Parameter(torch.rand(128, d_model, requires_grad=True))
        self.fc1.bias = nn.Parameter(torch.rand(128, requires_grad=True))

        self.norm2 = nn.LayerNorm(14)
        self.fc2 = nn.Linear(128, 14)
        self.fc2.weight = nn.Parameter(torch.rand(14, 128, requires_grad=True))
        self.fc2.bias = nn.Parameter(torch.rand(14, requires_grad=True))

        # self.fc3 = nn.Linear(32, 14)
        # self.fc3.weight = nn.Parameter(torch.rand(14, 32, requires_grad=True))
        # self.fc3.bias = nn.Parameter(torch.rand(14, requires_grad=True))
    ...
    def forward(self, in_put):
        ...
        output = self.activate_1(self.norm1(self.fc1(output)))
        output = self.activate_1(self.norm2(self.fc2(output)))
...
if __name__ == '__main__':
    ...
    for p in model1.parameters():  # 将fine-tuning 的参数的 requires_grad 设置为 True
        p.requires_grad = False

    optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=0.0002)  # AdamW: 论文采用的Adam扩展版本

    # scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=30, gamma=0.95)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', factor=0.8, patience=30, verbose=True, eps=1e-05)
    ...
    if os.path.exists('model_param_FCN.pt') == True:
        model.load_state_dict(torch.load('model_param_FCN.pt', map_location=device))
    ...
    for epoch in range(iteration):
        ...
        if epoch + 1 == 500:
            torch.save(model.state_dict(), 'model_param_FCN.pt')
