...
def Scaled_Dot_Product_Attention(Q, K, V, attn_mask, dropout=None):
    ...
    attn = F.softmax(scores, dim=-1)  # 求行的softmax
...
def make_batch(sentences_dict, mjd_dict, index):
    ...
    input_ids = [1] + sentences_dict[str(sentence_a_id)][0] + [2] + sentences_dict[str(sentence_a_id)][1] + [2] + \
                sentences_dict[str(sentence_a_id)][2] + \
                [2] + sentences_dict[str(sentence_a_id)][3] + [2] + sentences_dict[str(sentence_a_id)][4] + [2] + \
                sentences_dict[str(sentence_a_id)][5] + [2]
...
class MyDataset(Dataset):
    ...
    def __init__(self, data1, data2):
        ...
        self.flux, self.mjd_data, self.passend, self.masked_pos = zip(*data1)
        self.mjd_data = np.array(self.mjd_data)
        self.passend = np.array(self.passend)
        self.masked_pos = np.array(self.masked_pos)

        self.data2 = data2
        self.data2 = np.array(self.data2)

    def __getitem__(self, index):
        flux_ = torch.tensor(np.array(self.flux[index]))
        mjd_ = torch.from_numpy(np.array(self.mjd_data[index]))
        passend_ = torch.from_numpy(np.array(self.passend[index]))
        masked_pos_ = torch.from_numpy(np.array(self.masked_pos[index]))
        labels = torch.from_numpy(np.array(self.data2[index]))

        return flux_, mjd_, passend_, masked_pos_, labels
    ...
    def collate_fn(samples):
        flux = torch.stack([x[0] for x in samples])
        mjd = torch.stack([x[1] for x in samples])
        passend = torch.stack([x[2] for x in samples])
        masked_pos = torch.stack([x[3] for x in samples])
        labels = torch.stack([x[4] for x in samples])

        return flux, mjd, passend, masked_pos, labels
...
if __name__ =="__main__":
    ...
    train_loader = DataLoader(dataset=train_dataset, batch_size=16, shuffle=False, collate_fn=MyDataset.collate_fn)

    test_dataset = MyDataset(X_test, y_test)
    test_loader = DataLoader(dataset=test_dataset, batch_size=16, shuffle=False, collate_fn=MyDataset.collate_fn)
    ...
    for epoch in range(iteration):
        start = time.time()
        for i, (flux, mjd, passend, masked_pos, labels) in enumerate(train_loader):
            #         flux,  mjd, passend, masked_pos, labels = zip(*data)
            input_ids, passend_ids, mjd_ids, masked_pos, labels = flux.to(device), passend.to(device), mjd.to(
                device), masked_pos.to(device), labels.to(device)
            optimizer.zero_grad()
        ...
        for i, (flux, mjd, passend, masked_pos, labels) in enumerate(test_loader):
            input_ids, passend_ids, mjd_ids, masked_pos, labels = flux.to(device), passend.to(device), mjd.to(
                device), masked_pos.to(device), labels.to(device)
            prediction = model(input_ids, mjd_ids, passend_ids, masked_pos)
