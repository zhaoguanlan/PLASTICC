...
if __name__ == "__main__":
    ...
    print("开始运行程序,数据预处理中:...")
    path = '/home/guanlanzi/data/training_set.csv'

    #1.数据预处理
    start = time.time()
    sentences_dict, mjd_dict, id_dict = pre_process(path)

    s_d = open("sentences_dict_train.json", "w")
    json.dump(sentences_dict,s_d)
    s_d.close()

    m_d = open("mjd_dict_train.json", "w")
    ...
    print("train_data进行data_processing的时间为：{}".format(end-start))